<template>
	<el-main class="bg edit_wrap">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','job_number') || $check_field('add','job_number') || $check_field('set','job_number')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="工号" prop="job_number">
					<el-input id="job_number" v-model="form_sub['job_number']" placeholder="请输入工号"
							  v-if="user_group === '管理员' || (form_sub['staff_id'] && $check_field('set','job_number')) || (!form_sub['staff_id'] && $check_field('add','job_number'))"></el-input>
					<div v-else-if="$check_field('get','job_number')">{{form_sub['job_number']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','full_name') || $check_field('add','full_name') || $check_field('set','full_name')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="姓名" prop="full_name">
					<el-input id="full_name" v-model="form_sub['full_name']" placeholder="请输入姓名"
							  v-if="user_group === '管理员' || (form_sub['staff_id'] && $check_field('set','full_name')) || (!form_sub['staff_id'] && $check_field('add','full_name'))"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','gender') || $check_field('add','gender') || $check_field('set','gender')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="性别" prop="gender">
					<el-select id="gender" v-model="form_sub['gender']"
						v-if="user_group === '管理员' || (form_sub['staff_id'] && $check_field('set','gender')) || (!form_sub['staff_id'] && $check_field('add','gender'))">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','position') || $check_field('add','position') || $check_field('set','position')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="职位" prop="position">
					<el-input id="position" v-model="form_sub['position']" placeholder="请输入职位"
							  v-if="user_group === '管理员' || (form_sub['staff_id'] && $check_field('set','position')) || (!form_sub['staff_id'] && $check_field('add','position'))"></el-input>
					<div v-else-if="$check_field('get','position')">{{form_sub['position']}}</div>
				</el-form-item>
			</el-col>
...
<script>
	...
	export default {
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["staff_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?staff_id=" + form_sub["staff_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/staff/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/staff/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/staff/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/staff/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/staff/view','get');
